...
# Executor pequeño para no congelar la UI durante un webhook externo.
...
def build_interactive_html_report(
    ...
    def fig_html(fig):
        return pio.to_html(fig, full_html=False, include_plotlyjs="inline", config={"responsive": True, "displaylogo": False, "modeBarButtonsToRemove": ["lasso2d", "select2d"]})
...
def reset_database() -> None:
    """Borra absolutamente todos los datos guardados en SQLite para empezar de cero."""
    with db() as con:
...
def save_snapshot(
    ...
    """Guarda un corte con inserción masiva de pandas -> SQLite."""
    inv = inventory
    sal = sales
...
def downcast_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    de baja cardinalidad y enteros que realmente caben en chicos chicos.
...
def apply_redistribution_to_results(
    ...
    """Versión vectorizada: en vez de recorrer cada SKU con iterrows()
    (lento con catálogos grandes), se agregan las transferencias con
    groupby + merge, y las columnas se calculan de una sola vez con
    operaciones de pandas sobre el DataFrame completo."""
...
def build_alerts(df: pd.DataFrame) -> pd.DataFrame:
    """Versión vectorizada: en vez de recorrer fila por fila con
    iterrows() (muy lento con catálogos grandes), se arma cada tipo de
    alerta con una máscara booleana sobre todo el DataFrame a la vez."""
    if df.empty:
...
def post_webhook_async(url: str, payload: dict[str, Any]):
    """Dispara el webhook en segundo plano para no congelar la UI."""
    if not url.strip():
        return None
...
with st.sidebar:
    ...
    if st.button("🔄 Forzar recálculo", use_container_width=True, help="Limpia la caché interna y recalcula todo desde cero. Útil si editaste datos manualmente en la base."):
        st.cache_data.clear()
        st.toast("Caché limpiada · recalculando todo", icon="🔄")
        st.rerun()

    # --- NUEVO BOTÓN DE RESETEO TOTAL ---
    with st.expander("⚠️ Resetear Sistema"):
        st.warning("Borrará todo el historial y cortes guardados. Útil si te equivocaste de archivo.")
...
def build_dashboard_state(
    ...
    """Agrupa redistribución + ajuste + combinación + alertas en una sola
    función cacheada. Así, si `site_results` no cambió (no hay corte nuevo
    ni cambio de parámetros del motor), NEXUS no vuelve a correr estos
    cálculos en cada clic dentro de la app — solo se recalculan cuando
    realmente hay datos nuevos."""
...
transfers, site_results_adjusted, all_df, alerts = build_dashboard_state(site_results)
...
def render_automation_fragment(
    ...
    st.info("Arquitectura: NEXUS → webhook → n8n/Make/API → CRM + proveedor de WhatsApp. El proveedor externo gestiona plantillas y reglas de envío.")
...
# ============================================================
    ...
    st.caption("El ERP genera payloads para CRM, n8n/Make y WhatsApp por webhook.")
...
# ============================================================
    ...
    st.caption("Elige cómo quieres consumir la información: Excel operativo, Excel gerencial, HTML autocontenido e interactivo, PDF ejecutivo, CSV o JSON. El dashboard HTML ya no depende de un CDN externo para sus gráficos.")
    if all_df.empty:
